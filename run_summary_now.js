...
- "Ваше имя" → поле imya (только имя человека, например "Нариман", "Дида")
- "Что вызывает напряжение/вопрос" → поле vopros (только сам вопрос/проблема, без причин и шагов)
- "Почему важно" → поле pochemu (только причина важности)
- "Возможные шаги / Кто поможет" → поле shagi (только шаги или кто поможет)

Правила:
1. Каждая строка таблицы = отдельный tension объект
2. НЕ объединяй поля — каждое поле только своё содержимое
3. НЕ переноси "почему важно" в vopros
4. НЕ добавляй рекомендации в vopros — только то что написано в колонке
5. Если ячейка пустая — пустая строка ""
6. Не придумывай tensions которых нет в файле

Верни ТОЛЬКО валидный JSON-массив:
[{"imya":"Нариман","vopros":"Задачи в Jira фиксируются меньше чем обсуждается","pochemu":"Создаёт ощущение что работы меньше чем есть","shagi":"Фиксировать все задачи с ответственными","reshili":"нет","data":"","status":"🔴"}]
