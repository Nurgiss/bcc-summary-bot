...
(async () => {
  ...
  const [summaryResp, tensionsResp] = await Promise.all([
    openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: `Ты — ассистент дизайн-команды БЦЦ. Напиши полное структурированное саммари встречи НА РУССКОМ, строго следуя стилю и структуре эталона.

СТРУКТУРА САММАРИ (включи все секции из файла):
1. Заголовок с датой, ведущим, секретарём
2. 🏖️ Графики отпусков (если есть данные)
3. ✨ Изменения в дизайн-системе (если есть)
4. 👉 Обзор задач — по каждому дизайнеру отдельно: <b>Имя</b> — что сделано, что планируется
5. 📌 Пересечения и решения
6. 📢 Новости и обучение (если есть)
7. 🙏 Благодарности (если есть)

ПРАВИЛА:
- По каждому дизайнеру отдельный пункт с жирным именем
...
- Пиши всё что есть в файле, конкретно и по сути${exampleBlock}` },
