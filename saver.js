...
В протоколе есть таблица ❓ Tension с колонками:
- "Ваше имя" → это поле imya
- "Что вызывает напряжение/вопрос" → это поле vopros  
- "Почему важно" → это поле pochemu
- "Возможные шаги / Кто поможет" → это поле shagi

Также есть "Tension-ы с прошлой встречи" с теми же колонками + "Решили вопрос" и "Дата финала".

Правила извлечения:
1. Каждая строка таблицы = отдельный tension объект
2. НЕ объединяй поля — каждое поле только своё содержимое
3. НЕ переноси "почему важно" в vopros — это разные поля
4. НЕ добавляй рекомендации в vopros — только то что написано в колонке
5. imya = имя человека, кто поднял tension (например "Нариман", "Дида") — только имя, без лишнего текста
6. Если ячейка пустая — пустая строка ""
7. Не придумывай tensions которых нет в файле

Верни ТОЛЬКО валидный JSON-массив:
[{"imya":"Нариман","vopros":"Задачи в Jira фиксируются меньше чем обсуждается","pochemu":"Создаёт ощущение что работы меньше чем есть на самом деле","shagi":"Фиксировать все задачи с ответственными","reshili":"нет","data":"","status":"🔴"}]

Статус: 🔴 = критично/новое, 🟡 = в процессе/обсуждается/решили.
